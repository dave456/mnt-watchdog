...
Updated with a new version that runs in python. The idea was that using callbacks in the client would be a more efficient way to communicate with the indi server (as opposed to polling it), its unclear if this is an actual benefit as the callback mechanism gets spammed repeatedly in what looks like a polling loop running in the indi server. Both scripts use the same config file and use similar options.
